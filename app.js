    ...
    try {
        ...
        if (code) {
            try {
                ...
                if (codeInput) {
                    codeInput.value = '';
                    codeInput.placeholder = e.message === 'EXPIRED' 
                        ? (state.lang === 'de' ? 'CODE_ABGELAUFEN' : 'CODE_EXPIRED') 
                        : (state.lang === 'de' ? 'UNGÜLTIGER_CODE' : 'INVALID_CODE');
                    setTimeout(() => { codeInput.placeholder = (state.lang === 'de' ? 'CODE_EINGEBEN' : 'ENTER_CODE'); }, 2000);
                }
            }
        } else {
            await initGame(true);
...
async function initGame(createChallenge, isRestoring = false) {
    try {
        ...
        try {
            ...
            if (startBtn) {
                startBtn.innerText = e.message === "NO_QUESTIONS" 
                    ? (state.lang === "de" ? "FEHLER: LEERER_SEKTOR" : "ERROR: EMPTY_SECTOR") 
                    : (state.lang === "de" ? "FEHLER: SYSTEM_OFFLINE" : "ERROR: SYSTEM_OFFLINE");
                setTimeout(() => { startBtn.innerText = state.lang === 'de' ? 'PROTOKOLL_STARTEN' : 'INIT_PROTOCOL'; }, 3000);
            }
        }
    } catch (e) {
...
async function endGame() {
    try {
        ...
        if (state.isCreatingChallenge) {
            ...
            generateChallengeCode().then(code => { if (text) text.innerText = code; });
            ...
            const accuracy = state.questionCount > 0 ? Math.round((state.correctAnswers / state.questionCount) * 100) : 0;
            if (text) text.innerText = `FINAL_SCORE: ${state.score}\nPEAK_DATA: ${state.best}\nACCURACY: ${accuracy}%`;
        }
    ...
    try {
        ...
        if (startScreen && startScreen.classList.contains('active')) {
            ...
            if (eye) {
                ...
                setTimeout(() => { if (eye) eye.src = './assets/images/ack_core_clean.png'; }, 150);
            }
        }
    } catch (e) {}
}, 4000);
    ...
    try {
        ...
        visibilityTimeout = setTimeout(() => {
            if (document.hidden) {
                ...
                const gameScreen = document.getElementById('game-screen');
                if (state.sessionActive && !state.isPaused && gameScreen && gameScreen.classList.contains('active')) {
...
if ('serviceWorker' in navigator) {
    try {
        ...
        window.addEventListener('beforeunload', () => { 
            if (leaderTab) {
                try { localStorage.removeItem('sw_leader'); } catch (e) {}
            }
        });
...
async function showLeaderboard() {
    try {
        ...
        if (entriesDiv) entriesDiv.innerHTML = '<div style="padding:20px;text-align:center;">CONNECTING...</div>';
        ...
        try {
            ...
            if (entriesDiv) entriesDiv.innerHTML = '<div style="padding:20px;text-align:center;color:var(--error);">SERVER_UNAVAILABLE</div>';
        }
    } catch (e) {
