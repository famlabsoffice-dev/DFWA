...
describe('StorageManager', () => {
  ...
  describe('getSignature', () => {
    test('should generate a consistent signature for given data and secret', async () => {
      ...
      const expectedMockSignature = Array.from(
        new TextEncoder().encode(`mock_signature_for_test_data_HMAC_SHA-256`)
      )
    ...
    test('should throw error if no secret is provided', async () => {
      const data = 'test_data';
      await expect(StorageManager.getSignature(data)).rejects.toThrow(
        'SYSTEM_SECRET not provided for StorageManager'
      );
    });
  });
