...
export const StorageManager = {
    ...
    async saveSecure(key, value, secret) {
        try {
            ...
            UIManager.showModal('Storage Error', 'Could not save game data securely. Your progress might be lost.', 'var(--error)');
        }
    },
            ...
            try {
                ...
                if (val && sig) {
                    ...
                    if (sig !== expected) {
                        ...
                        onFail(key);
                        UIManager.showModal('Security Alert', `Data integrity check failed for ${key}. Local data has been reset.`, 'var(--warning)');
                    }
                }
            } catch (e) {
